...
jQuery(function($) {
    ...
    try {
        ...
        $(audio).on('play', function() {
        	...
        	if (h.className.indexOf('autoplay-disabled') != -1) {
        		h.className = h.className.replace('autoplay-disabled', "autoplay-enabled");
        	} else {
        		h.className += " autoplay-enabled";
        	}
        });
        
        audio.src = src;
        
        // Set a timeout for it to fail in case it doesnt do an error
        setTimeout(function () {
        	if (h.className.indexOf('autoplay-enabled') == -1 &&
        		h.className.indexOf('autoplay-disabled') == -1) {
    			console.log("Autoplay DISABLED.");
        		h.className += " autoplay-disabled";		
    		}
        }, 500); 
    	...
    	console.log("Autoplay DISABLED.", e);
    }
}); 